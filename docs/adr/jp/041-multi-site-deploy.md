...
**承認済み（Accepted）** — 2026-07-19 実装。ADR-031（マルチ環境デプロイ）を置換せず**拡張**する。環境軸（dev / prd）の内側にサイト軸を追加する。
...
- サイトスタックは **wave 単位で並列デプロイ**（カナリア 1 サイト → 以降
  `deployConcurrency` サイトずつ、各 wave は前 wave の完了を待つ。既定 2、
  1 で直列）。ECS のローリング更新は新旧タスクを併走させるため、接続数バジェットは
  「同時更新は `deployConcurrency` サイト」を前提に、消費の大きい順にその数だけ
  新タスク分（`minSize` 台 × プール）を加算する — テンプレートが `DesiredCount` を
  `minSize` に固定しているため、デプロイ時に希望数は `minSize` へ戻り（負荷中は
  オートスケールが数分で戻す。意図した挙動）、旧タスク分は定常側で計上済み。
  この固定を外す場合はバジェットも `maxSize` 基準に戻すこと。収まらなければ
  synth エラー（当初は直列固定で、Wave 並列化を
  将来最適化としていた — 2026-09-08 に実装）
...
| **中間（段階的最適化）**    | ALB はまずサイト別、共用は第二段階（ADR-049 で確定 — ヘッダールーティング）                                                                                                                                                                                            |
...
- **パイプライン所要時間**: 1 push あたり「dev のサイト数 + prd のサイト数」分のデプロイが流れる。カナリア後は `deployConcurrency` サイトずつの wave で並列化され（既定 2、1 で直列）、接続数バジェットはその数だけ同時更新分を計上する。dev のサイト数の絞り込みも併用する
...
- ADR-049（マルチサイトの ALB 統合）: 本 ADR が第二段階として残した ALB 共用を確定する
