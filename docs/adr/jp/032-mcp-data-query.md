...
3. **Parquet の取り込み**: storage アダプタでプレビュー Parquet（`preview_key`）を取得し、
   一時ファイルへ書き出す。`previewKey` をキーに lru-cache（ADR-004）で一時ファイルをキャッシュし、
   同一リソースへの連続クエリでの再ダウンロードを避ける。
4. **サンドボックス（本 ADR の核心）**: クエリごとに**使い捨ての DuckDB インスタンス**を作り、
   ...
   4. **行数・時間の上限**:
      ...
      - **ウォールクロックのタイムアウト**で接続を `interrupt()` する（DuckDB に SQL レベルの
        statement_timeout がないため、タイムアウトで明示キャンセルする）。
   5. クエリ終了後はインスタンスを破棄する。
5. **同時実行**: セマフォで同時クエリ数を制限し、メモリ使用（1 クエリあたり最大 50MB のテーブル +
   クエリ作業メモリ）を境界づける。超過分はキューイングまたは 429 を返す。
6. **アクセス制御**: プレビュー（ADR-017）と同じ `getByIdWithAccessCheck` を通す。
...
- **API**: `packages/api` に `@duckdb/node-api` 依存と、クエリ実行サービス
  （サンドボックス・キャッシュ・セマフォ・タイムアウト）を追加。`/schema`・`/query` ルートと
  MCP ツール 2 種を追加。
- **デプロイ**: `@duckdb/node-api` はネイティブアドオン。web コンテナ（ECS Fargate、ADR-020）の
  Docker ビルドに含める必要がある。メモリ上限はクエリサンドボックスと ECS タスク定義の両方で設定する。
...
4. **マテリアライズ済みインスタンスのキャッシュ**: 一時ファイルだけでなく、ロック済み DuckDB 接続自体を
   リソース単位でキャッシュして連続クエリのレイテンシを下げる（同時実行管理が複雑化するため v2）。
5. **結果フォーマット**: MCP 返却を JSON 行 / Markdown テーブル / CSV のいずれにするか、トークン効率の観点で精査。
6. **レート制限・課金**: AI からの高頻度・高コストクエリに対するレート制限とコスト可視化。
