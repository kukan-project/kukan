...
| 種別                                        | 該当               | DB キューで解決するか |
| ------------------------------------------- | ------------------ | --------------------- |
| A. 同じ行・同じオブジェクトへの二重書き込み | #180 / #181 / #182 | **しない**            |
| B. トランザクション外の副作用               | #183 / #184        | **する**              |

B は DB キューの本領である。enqueue が `INSERT` になるため、業務データの更新と同一
トランザクションに入り、「コミットしたのに投入が消える」（#183）が構造的に起きなくなる。

A は解けない。#181 は HTTP リクエスト 2 本の競合でキューを通らず、#182 は API の設計判断、
#180 の根は「2 つの実行が同じリソースを書く」ことである。リソース単位の singleton key で
重なりは減るが、リース方式である以上ワーカー停止後に古い実行が再開する窓は残り、
CAS は結局必要になる。**発生頻度が下がるだけで、対処は省けない。**

実際、対処に手間がかかったのは A の 3 件（うち 2 件が P1）だった。
