...
## 再検討（2026-07-28）— 競合の正しさという観点

ADR-043 の実装中に並行実行由来の不具合が複数見つかったことを受けて、
「DB バックエンドのキュー（pg-boss 等）に替えれば競合の対処が楽になるか」を再検討した。
**結論は変わらず、SQS を維持する。**

当時の取り下げ理由はスループットと構成の簡素化に関するもので、正しさの観点は評価されていなかった。

### 見つかった競合は 2 種類に分かれる

| 種別                                        | 該当                                                 | DB キューで解決するか |
| ------------------------------------------- | ---------------------------------------------------- | --------------------- |
| A. 同じ行・同じオブジェクトへの二重書き込み | 版の逆転、アップロード鍵の滞留、メタデータの先行反映 | **しない**            |
| B. トランザクション外の副作用               | 投入の消失、オブジェクトの追跡漏れ                   | **する**              |

B は DB キューの本領である。enqueue が `INSERT` になるため、業務データの更新と同一
トランザクションに入り、「コミットしたのに投入が消える」が構造的に起きなくなる。

A は解けない。アップロード鍵の滞留は HTTP リクエスト 2 本の競合でキューを通らず、
メタデータの先行反映は API の設計判断、版の逆転の根は「2 つの実行が同じリソースを書く」
ことである。リソース単位の singleton key で重なりは減るが、リース方式である以上ワーカー停止後に古い実行が再開する窓は残り、
CAS は結局必要になる。**発生頻度が下がるだけで、対処は省けない。**

実際、対処に手間がかかったのは A のほうだった。

### 取り下げ理由は今も有効

最大の障害である Aurora Serverless v2 の 0 ACU は、pg-boss に限らず
**DB バックエンドのキューが接続を保持し続ける**以上そのまま当てはまる。
ADR-041 でマルチサイトに進んだ後は、サイト数だけ倍加する。

### B の利点は移行せずに取れる

enqueue をトランザクションに入れる価値は、outbox パターンで代替できる。

```
投入の意図を DB に書く（業務データの更新と同一トランザクション）
  → コミット後に即座に enqueue を試みる（通常経路・低レイテンシ）
  → 取りこぼしは定期スイープが拾う（保証）
```

スイープ間隔が縛るのは失敗時だけで、通常経路は即時のまま。
worker が複数台ある場合、cron はプロセスごとに走る（`protect: true` はプロセス内の
重複しか防がない）ため、claim には `FOR UPDATE SKIP LOCKED` が要る。

なお Lake 再投入については、投入の意図が既に
`resource_version.ducklake_snapshot_id IS NULL` として永続化されており、
専用の outbox テーブルすら不要な可能性がある（既存の `pendingLakeIngestQuery` の再投入で足りる）。

## 関連 ADR

- ADR-002: SQS over BullMQ（現行の判断）
- ADR-005: 4 アダプターのみ（QueueAdapter が対象の 1 つ）
- ADR-043: リソースバージョニング（再検討のきっかけとなった競合の出どころ）
