...
| TXT          | `null`（`/raw` エンドポイントを別途使用）                 | テキスト表示         |
| その他       | `null`                                                    | 「利用不可」表示     |
...
ResourcePreview
  ...
  ├── TXT  → TextPreview  → /raw → <pre>
  └── *    → TablePreview → preview-url → hyparquet table
```

### `download-url` との役割分担

| エンドポイント | 用途                                 | Content-Disposition |
| -------------- | ------------------------------------ | ------------------- |
| `preview-url`  | ブラウザ内プレビュー表示用           | inline              |
| `download-url` | ユーザーによるファイルダウンロード用 | attachment          |

PDF の場合、両者は同じファイルを指すが disposition が異なる。
