...
Layer 1: Canonical version files (all formats)
         versions/{packageId}/{resourceId}/v{n}.{token} — immutable, tracked by resource_version
...
2. When content arrives — an upload being promoted, or a Fetch downloading — the live object
   is **copied** under `versions/`.

   |         | Key                                              | Who points at it               |
   | ------- | ------------------------------------------------ | ------------------------------ |
   | live    | `resources/{packageId}/{resourceId}.{token}`     | `resource.storage_key`         |
   | version | `versions/{packageId}/{resourceId}/v{n}.{token}` | `resource_version.storage_key` |
   ...
   **Creating a version** means only this — copy and number (`createVersion` in the code). It
   reads nothing, not even whether the content is tabular (ADR-046).
   ...
   **A version begins to exist when its row lands, not when the copy is made.**
   ...
   2. reserve  record the key about to be written (ADR-045)
   3. copy     live object → the version's key
   4. insert   the row, dropping the write-ahead record in the same statement
   ```

   What exists after 3 is an object recorded as being written and nothing more. Die between 3
   and 4 and the key expires referenced by no one, and the sweep collects it — **that version
   never existed**. So "numbering" means the number going onto the row; the `v5` inside the key
   is a copy of what the row decided. Nothing reads a version number out of a key.

   **The copy is historical, not structural.** Pointing a version straight at the live object
   would remove steps 2 and 3 above entirely, leaving the birth of a version as one statement.
   What it saves is one object's worth of storage (the duplication does not scale with version
   count — it is always **one version's worth**) and one server-side copy per version.

   **Immutability is not the reason.** Live objects are as unchanging as version files: every
   writer mints a new key, and `promoteUpload` merely **re-points** at the uploaded object.
   Nothing overwrites an object in place. Nor does anything depend on splitting hot from cold
   by prefix — the bucket has two lifecycle rules and neither looks at one.

   What it would actually cost is two things:

   - **Reclamation booking needs one more predicate** — never book a key a version points at.
     Without it, **the survival of canonical version files** rides on the completeness of the
     sweep's reference check (ADR-045 §3)
   - **An unchanged re-fetch needs cleaning up after.** Fetch always writes a new key and
     publishes unconditionally; whether a version is made is decided afterwards. The key it
     wrote never becomes a version, so unless live is re-pointed at the existing version's key
     and that one dropped, the saving evaporates

   **"Live with no version" mostly disappears.** What does the work is the **order**, not the
   prefix. The state exists today because Fetch **publishes live before any version is made**;
   under a link, the download lands in the version's place to begin with, and the row insert and
   the pointer move both go after it.

   | Situation                         | Nature     | Under a link                                               |
   | --------------------------------- | ---------- | ---------------------------------------------------------- |
   | Just after a revert               | structural | **gone** — live points at the restored version             |
   | An unchanged re-fetch             | structural | **gone** — a key that became no version is never published |
   | Content a killed run left behind  | structural | **gone** — the publish follows the version                 |
   | Resources predating the backfill  | legacy     | **gone** once the backfill gives them a v1                 |
   | Promotion until the version lands | structural | **stays** — see below                                      |

   Only uploads keep it. `promoteUpload` re-points live on the HTTP request (outside the claim,
   ADR-044 §6) while the hash is measured by the pipeline, so the window from promotion to the
   version cannot close — and it becomes permanent if that pipeline never completes.

   **Changing the order also puts the row insert and the pointer move in one statement.** Both
   are PostgreSQL writes, and nothing in storage sits between them any more. That is impossible
   today because of the copy — and it is the same reason a revert has to step off, restore, and
   put the step-off back when the restore fails. **A link removes that two-step entirely.**

   **The only reason not to do it now is reach** — it touches Fetch, the pointer move,
   reclamation, purge and revert (open issue 10).
...
| **version file**   | one object under `versions/` | the version's contents                                                                              |
...
when and why (a headstone). A revert destroys neither: it only takes away standing.
...
and from there the `state` moves in exactly four ways.
...
```
  ...
  └────────┘                       └────────────┘
       ...
       └────────────────┬───────────────────┘
                        ...
                        ▼
                  ┌─────────┐
                  │ purging │  a durable claim (ADR-028). The worker destroys every
                  └─────────┘  copy; a failure retries from this same state
                        │ layer 1, layer 2 and the derivatives are all gone
                        ▼
                  ┌─────────┐
...
What goes `active → superseded` is every version **above where the revert is going**. A revert
lands on "the newest active version", so leaving those active would have it land on one of them.

There is no way back out of `purging`. A purge is a mechanism that does not give up partway;
made reversible, it could **report success with a legal deletion left unfinished**.

| State        | Live candidate | Version file | Layer-2 snapshot        | Purgeable |
| ------------ | :------------: | ------------ | ----------------------- | :-------: |
| `active`     |      yes       | yes          | retained                |    yes    |
| `superseded` |       no       | yes          | retained (still diffed) |    yes    |
| `purging`    |       no       | being erased | expired                 |     —     |
| `purged`     |       no       | none         | none                    |     —     |

- **A superseded version can still be purged on its own.** Content captured from a file that
  should never have been served survives a revert, so the path to destroying it cannot close
- **Superseded snapshots are retained.** A diff resolves two versions to their snapshots and
  reads both, so expiring the one a revert stepped off breaks comparing against it (§6-4)
- **`purging` ones are not.** A purge calls the expiry from inside its own run, before it can
  set the row to `purged`; retaining a row mid-purge would **leave the purged version's files
  on disk** (§6-4)
...
3. **DuckDB reads and writes storage directly** (httpfs / S3 API). An exception to the
   ...
   The scope is DuckLake's own `DATA_PATH` (the `lake/` prefix). `resources/`, `versions/`
   and `previews/` stay behind the adapter — Interpret too pulls the version file with
...
6. **Diff extraction uses a three-tier fallback**:
   - **With a primary key** → row-level diff via `MERGE`. Added, deleted, and **changed**
     rows are distinguished, with minimal-cost history. The primary-key columns are
     optionally designated by an administrator in resource settings (v1 is manual only;
     AI-suggested candidates are a future extension of the ADR-040 suggestion
     infrastructure).
...
3. **Affected locations**:
   - Layer 1: delete the version file (`versions/.../v{n}.{token}`)
...
Two conditions put them together, and both cost the same thing in shape — rebuilding from
layer 1 — differing only in reach.

| What makes it shared                           | Cost at purge                            |
| ---------------------------------------------- | ---------------------------------------- |
| Compaction merges across versions (§6-2)       | Rebuild every merged version             |
| Delta writes mix history-only and current rows | Re-ingest from the purged version onward |

**Turning the inlined table off is exactly because its cost is the worst of the three**
(§6-1): with it on, neither expiry nor cleanup reaches an inlined row, and the only way to
reclaim one is `DROP TABLE` — a rebuild of every version.

**Whole-table replacement is the one shape that shares no container**, which is exactly why a
purge costs it nothing but expiry plus cleanup. Measurements agreed — with either compaction
applied or inlining enabled, "roll back and re-ingest only later versions" fails, and only a
...
as the **degenerate case of a primary-keyed diff**, and the purge mechanism is chosen **per range
...
primary-key selection UI yet. Once primary keys can be specified, both shapes appear in the same catalog
...
4. **Expiry uses an explicit list** (`versions => [...]`).
   ...
   The candidates are every snapshot minus those
   referenced by a **surviving version** (`state` of `active` or `superseded`) minus the
   newest snapshot. A time-based `older_than` cannot be used — the ids are one
   catalog-wide sequence, so an age cutoff sweeps up snapshots belonging to resources that
   simply have not changed. The newest is always kept because a purge that rolled a table
   back has just created one that no version record points at yet.

   **`active` alone is not enough. `superseded`** — what a revert stepped off from above its
   destination — keeps its snapshot too, because a snapshot outlives being the current
   contents: a diff resolves two versions to their snapshots and reads both, so expiring the
   one that was stepped off breaks comparing against it. `purging`, conversely, is excluded
   alongside `purged`: a version purge calls this from inside its own run, before it can
   set the row to `purged` (that write also nulls the snapshot), so retaining a row
   mid-purge would leave **the purged version's files on disk** with nothing but a package
   or organization purge able to reach them.
...
- **Backward compatibility**: Existing resources get v1 at their next replacement. Existing
  ...
  > after it ships. It runs from the admin screen, copying the current file of every
  > version-less resource as v1, and takes the execution claim (ADR-044 §1).
...
6. **AI primary-key suggestion**: Add primary-key candidate suggestion to the ADR-040
   ...
   tier)
7. **The ii-b purge mechanism (a prerequisite for graduating)**: Primary-keyed diffs break the §5
...
9. **Iceberg export**: Provide public snapshots in Iceberg format (direct reads from
   ...
   limited
10. **Make live a link to a version** (§1-2): the write-ahead record and the copy drop out of
    creating a version, leaving it one statement. **No structural obstacle has been found.**
    - **Immutability is not a reason** — live objects do not change either (§1-2)
    - **Purge argues for sharing, not against it** — a version and its live copy already go in
      one operation (delete the version file, book the old live key for reclamation), and one
      object is one fewer thing to forget. A failed rollback would leave live pointing at a
      deleted object — unservable, which is the safe direction for a legal deletion
    - **Versions still never share with each other** — each holds the key it had while it was
      live, so ADR-046 §3's "one version, one object" survives

    The cost is the three items in §1-2 (two kinds of referent, one predicate on reclamation
    booking, cleanup after an unchanged re-fetch). It **touches Fetch, the pointer move,
    reclamation, purge and revert**, so it belongs after ADR-044 / 046 have settled, as its own
    change
