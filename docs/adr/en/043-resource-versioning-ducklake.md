...
2. When content arrives — an upload being promoted, or a Fetch downloading — a version is
   ...
   **When it copies, live moves to that key too.** This is an invariant that has to hold when
   creating a version finishes — **the live pointer and the version just created name the same
   object**. Left behind, live points at an object no surviving version owns, and purging the
   older version that does own it **deletes what live is serving**.

   **This is not a rule that "live is the newest active version".** A purge identifies live by
   looking up the object's owner (§1.1, and `isLive` in spec §9.6), never by version order —
   mid-purge the object live names is owned by a `purging` version. What is claimed here is
   only that the two agree **immediately after a create**; which version owns live afterwards
   changes every time the pointer moves.
   ...
   **A version begins to exist when its row lands, not when the copy is made.**
   ...
   ```
   1. decide  does **version identity** differ from the previous version
              (`sameVersionIdentity`, §1-4), and is the live pointer still this run's?
...
3. Add a `resource_version` table (managed by Drizzle):
   - `id` (UUID), `resource_id` (FK), `version` (sequential), `storage_key`, `size`,
     `hash`, `origin` (`upload` | `fetch` | `revert`), `restored_from` (the version number a
     revert named; null on every other path — versions with the same bytes and interpretation can
     repeat, so it cannot be derived),
...
4. **A replacement that is the same bytes read the same way does not create a version**: if
   **version identity** matches the newest active version, issue no new version and only update
   a verification timestamp (`sameVersionIdentity`; its inputs grow in steps, see the table below).
   ...
   > even when the bytes do not change**.
   >
   > **So "matches" is asked of every input to version identity, and the inputs grow in steps:**
   >
   > | Stage | Inputs to version identity                |
   > | ----- | ----------------------------------------- |
   > | today | hash, format                              |
   > | ii-b  | + **the primary key columns** (spec §6.4) |
   > | ii-c  | + the settled types (spec §6.5)           |
   >
   > **Asked through one function rather than by enumerating columns**
   > (`sameVersionIdentity`, `@kukan/shared`). A revert's "already at the destination" goes
   > through the same one (ADR-044 §4), so an added input cannot leave one of them comparing the
   > old set — and if it did, a destination differing only in its key could not be restored.
   >
   > **In ii-b the primary key does exist at creation time.** The setting lives on the resource
   > (`resource.column_settings`) and is decided before content arrives, so the original reason
   > for keeping it out of this gate — that it did not exist yet — does not hold from ii-b on.
   > The settled types join the same way in ii-c.
...
A version record only ever comes into being through §1-2, and three things do it: the pipeline,
when a replacement or a fetch differs from the previous version; the one-time migration backfill,
which gives a v1 to resources holding no version at all; and **a revert** (ADR-044 §4), which
issues the content of the version it names as a new one. **Only the revert's content comes from
inside the same resource** — the number it came from is recorded in `restored_from` (below).
...
| State     | Automatic fallback | Version file | Layer-2 snapshot | Purgeable |
| --------- | :----------------: | ------------ | ---------------- | :-------: |
| `active`  |        yes         | yes          | retained         |    yes    |
| `purging` |         no         | being erased | expired          |     —     |
| `purged`  |         no         | none         | null             |     —     |

- **"Live" and "where a purge falls back to" are different questions.** Live is **the version
...
1. When the pipeline Fetch runs (manual reprocess; scheduled re-fetch in the future), store
   the content as `v{n}` in Layer 1 **only if its version identity differs** from the newest
   active version (the same gate as §1-4, the same function). If it matches, only update the
