...
同一のアプリケーションコード + Dockerイメージ
       │
       ├─► AWS App Runner (API/Web) + ECS Fargate (Worker) → クラウド（標準）
...
export const profiles = {
  ...
  // 中規模（AWS App Runner + ECS Fargate — 標準推奨）
  'aws-standard': {
    runtime: 'node', // App Runner (api/web) + ECS Fargate (worker)
...
API/Web は App Runner、Worker は ECS Fargate で運用する。
...
- App Runner は HTTP リクエスト駆動が前提で、ロングポーリングプロセスに不向き
