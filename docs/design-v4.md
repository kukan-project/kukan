...
CKANは世界中の政府・自治体で利用されるオープンデータカタログの事実上の標準であるが、
以下の構造的課題を抱えている:
...
- プレビュー機能はIngest時の事前生成+S3キャッシュで高品質に提供する
...
同一のアプリケーションコード + Dockerイメージ
       │
       ├─► AWS App Runner          → クラウド（標準）
       ├─► Docker Compose          → オンプレ / 閉域網
       ├─► ECS Fargate / EKS       → 大規模クラウド（将来）
...
| XLoader/DataPusher+ | 自動DataStore投入 | 廃止 → Ingest Workerに統合 |
...
| キュー | SQS (AWS) / InProcess (開発・オンプレ) | イベント駆動Ingest。Redis不要 |
...
> v3で使用していたRedis/BullMQはv4でSQS + InProcess方式に移行し、Redis依存を完全に排除。
...
│  SearchService    IngestService     HarvestService          │
...
2. **コネクションプール1つ** — App Runnerスケールアウト時の接続数爆発を防ぐ
...
export const profiles = {
  ...
  'development': {
    ...
    queue: 'in-process',         // SQS不要。APIプロセス内で直接処理
    ai: 'none',
  ...
  // 小規模（最小コスト）
  'small': {
    runtime: 'node',             // App Runner or Docker Compose
    database: 'aurora-serverless',  // or postgres
    search: 'postgres',          // OpenSearch なし
    storage: 's3',               // or minio
    queue: 'in-process',         // 日に数回の処理なら十分
    ai: 'none',                  // or bedrock
    auth: 'better-auth',         // + OIDC プラグイン（外部IdP連携時）
    scheduler: 'node-cron',      // 品質チェック定期実行（プロセス内）
  },

  // 中規模（AWS App Runner — 標準推奨）
  'aws-standard': {
    runtime: 'node',             // App Runner × 2 (api + worker)
  ...
  'on-premise': {
    ...
    storage: 'minio',
    queue: 'in-process',         // or bullmq（大規模オンプレ時）
    ai: 'ollama',                // or 'none'
...
│  │                                                    │    │
│  │  ┌──────────────────┐  ┌────────────────────────┐ │    │
│  │  │ App Runner       │  │ Aurora Serverless v2   │ │    │
│  │  │ "worker"         │  │ (PostgreSQL)           │ │    │
│  │  │                  │  │                        │ │    │
│  │  │ Ingest Worker    │  │ 0.5〜N ACU             │ │    │
│  │  │ (SQSポーリング)   │  └────────────────────────┘ │    │
│  │  │                  │                              │    │
│  │  │ 1vCPU / 2GB      │                              │    │
│  │  │ min:1 max:5      │                              │    │
...
- ALB不要（内蔵）、NAT Gateway不要 → Fargateより実運用コストが安い
- ECR pushのみで自動デプロイ（ECSのタスク定義等の管理不要）
- 常駐プロセス可能（WorkerのSQSロングポーリングがそのまま動く）
- 非アクティブ時にvCPU課金停止 → 自治体の夜間・休日コスト削減
- 25インスタンス上限あるが、自治体規模では十分（国レベルは別途EKS検討）
- 将来Fargateへの移行は同一Dockerイメージで可能（IaC変更のみ）
...
特徴:
  - Redis不要 → InProcessキュー（APIプロセス内でIngest処理）
...
**AWS App Runner構成（中規模・OpenSearchあり）**

| 構成要素 | 月額概算 |
|---------|---------|
| App Runner "api" (0.25vCPU/0.5GB) | ~$5 |
| App Runner "worker" (1vCPU/2GB) | ~$14 |
...
※v3（Redis/ElastiCache込み ~$160/月）から大幅削減
※App Runnerは非アクティブ時にvCPU課金停止のため、夜間・休日のコストがさらに軽減
※ALB不要、NAT Gateway不要のため、Fargate構成比で約$85/月安い
...
|------|-------------|-----------|
| development | InProcessQueueAdapter | なし（APIプロセス内で処理） |
| small | InProcessQueueAdapter | なし |
| aws-standard | **SqsQueueAdapter** | **App Runner Worker 別サービス** |
| aws-large | **SqsQueueAdapter** | **ECS/EKS Worker** |
| on-premise | InProcessQueueAdapter | なし（大規模時はBullMQ検討） |
...
class InProcessQueueAdapter implements QueueAdapter {
  async send(job) {
    // 同一プロセス内で即座にハンドラーを呼び出し（非同期）
    setImmediate(() => this.handler?.(job));
  }
  consume(handler) {
...
// --- BullMQ実装 (大規模オンプレ・将来) ---
class BullMQQueueAdapter implements QueueAdapter { /* Redis + BullMQ */ }
```
...
```typescript
class OpenAIAdapter implements AIAdapter { /* OpenAI API / Azure OpenAI */ }
class BedrockAdapter implements AIAdapter { /* AWS Bedrock */ }
class OllamaAdapter implements AIAdapter { /* ローカル LLM (閉域網用) */ }
...
```typescript
interface IngestStep<TInput, TOutput> {
  name: string;
  execute(input: TInput, ctx: ServiceContext): Promise<TOutput>;
}

// Analyze → Extract → Preview → AI → Index
const INGEST_PIPELINE = [analyzeStep, extractStep, previewStep, aiStep, indexStep];
...
async function processResource(resourceId: string, ctx: ServiceContext) {
  ...
  try {
    ...
    await db.update(schema.resources)
      .set({ ingestStatus: 'error', ingestError: String(err) })
      .where(eq(schema.resources.id, resourceId));
...
ユーザーがファイルをアップロード
  ...
  アップロード完了通知
       │
       ▼
  QueueAdapter.send({ type: 'ingest', payload: { resourceId } })
       │
       ├─ [AWS] SQS キュー → App Runner "worker" がロングポーリングで受信
       │                     → processResource(resourceId) 実行
       │
       └─ [開発/オンプレ] InProcessQueue → 同一プロセス内で即座に実行
       │
       ├── Step 1: Analyze（軽量）
...
app.post('/datasets/:id/resources', async (c) => {
  ...
  const resource = await db.insert(schema.resources).values({
    datasetId, s3Key, status: 'pending',
  }).returning();
...
Ingest時のスマートパーサー（7.6節）と同じロジックを再利用。
...
interface CsvIssue {
  type: 'column_mismatch'           // 行ごとの列数不一致
...
AIAdapter を使ったPII（個人情報）スキャン。Ingest時（9.2節）に加え、
定期的な再スキャンで見逃しを防ぐ。
...
| 環境 | スケジューリング方式 | 実行先 |
|------|-------------------|-------|
| aws-standard | **EventBridge Scheduler** → SQS → Worker | App Runner "worker" |
...
SQS キュー
    ...
    │
    ▼
App Runner "worker" → QueueAdapter.consume() で受信
    │
    ▼
...
Ingestパイプラインのスマートパーサー（7.6節）とも共有する。
...
Data Editor上のテーブル（承認済み）
  ...
  ④ QueueAdapter.send({ type: 'ingest' })
  ⑤ Ingestパイプライン実行（ただし既にバリデーション済みなので高速）
  ⑥ OpenSearchインデックス更新
    │
    ▼
カタログ上にデータセットとして公開
...
interface CatalogPlugin {
  ...
  hooks?: {
    ...
    'resource:afterIngest'?: (resource: Resource, result: IngestResult) => void;
...
**廃止**: datastore_* 系 → 代替: /api/v1/resources/{id}/rows, /search 等
...
| XLoader / DataPusher+ | Ingest Worker（QueueAdapter + ステップ分割） |
...
| Redis / BullMQ（v3） | SQS + InProcess + lru-cache |
...
- **`apps/editor`** フロントエンドUI
  ...
  - 承認フローUI
- **カタログ連携**
  - ワンクリック公開（Editor → S3 → Ingest → カタログ）
...
| App Runner 25インスタンス上限 | 国レベルのみ → ECS/EKS移行（同一イメージ） |
...
*v4 — Turborepoモノレポ構成、Better Auth認証基盤、Redis排除（SQS + InProcess + lru-cache）、
