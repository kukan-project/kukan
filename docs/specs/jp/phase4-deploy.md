...
3. DDL 側の接続には `lock_timeout`（5 秒）が入っており、`ACCESS EXCLUSIVE` を待たされ続けて
   その後ろに当該テーブルへの問い合わせが積み上がる事態を避ける。タイムアウトしたら 5 秒置いて
   同じプロセス内でやり直す（最大 12 回、約 2 分）。塞いでいるのはたいてい正当なセッション
   （旧イメージの Worker が取込中に `resource_version` の `ACCESS SHARE` を数十秒保持する等）で、
   初回のタイムアウトで落ちるタスクは次の起動でも落ち、3 回連続で ECS の circuit breaker が
   デプロイをロールバックしてしまうため。間を置くのは、失敗した試行の後ろに並んだ問い合わせを
...
`account` テーブルから互換用の列を落とすマイグレーションが 2 つ入っている。適用後の DB に対して
**それより前のイメージは認証が動かない**。

1. 1.6 互換のシム（`expiresAt` 列と `issuer` のデフォルト）を落とす（v0.22.0）。それより前のイメージは
   サインイン・サインアップが `column "expiresAt" does not exist` で 500 になる
2. `issuer` 列と `(issuer, accountId)` の一意インデックスを落とす。better-auth 1.7.3 がこの列の要求を
   撤回して `(providerId, accountId)` によるアカウント識別に戻し、書き込まなくなったため。
   v0.17.0 から v0.28.0 までのイメージはこの列でアカウントを引くので、同じく認証が動かない
