...
> **実装完了（2026-07-25）。記録である。** 実装後に ADR-043 §1 が版とオブジェクトの関係を、
> ADR-046 がステップの順序と解釈の担い手を変えた。版は複製ではなく作成元のオブジェクトを
> 所有し、正本の鍵は 1 種類（`resources/…`）である。現在の姿は `docs/pipeline.md` を参照。
> 以下のファイルパス・鍵の形・ステップ名は当時のものである。

> 🔴 **パージの意味論は置き換わった。** 本書は随所でパージを「**法的削除**」と呼んでいるが、
> **その語はもう使わない**。ADR-043 §5 と `phase-versioning-2-ducklake.md` §9 が主張範囲を
> 狭めており、**保証するのは「製品から即時に取得できなくすること」だけである。層 2 の有無に
> よらない。**
>
> **動作そのものは本書のままである。** 消えるものと、いつ消えるかは次のとおり:
>
> - **版ファイルは削除する**（層 1）。ただし**バイト列がその場で消えるわけではない** —
>   §8.3 のとおり、S3 バージョニングの noncurrent 版はライフサイクル（30 日）まで、
>   AWS Backup の復旧ポイントは保持期間まで残る
> - **プレビューと検索インデックスは、対象が live 版のときだけ**捨てる（§8.2）。中間版の
>   パージでは触らない — それらは配信中の内容を表しており、中間版のパージはそれを変えない
> - **層 2（DuckLake、Phase ii で追加）には行が残りうる。** スナップショットがカタログ全体の
>   ものなので、別リソースのために保持している履歴が同じファイルを掴む。**こちらには期限が無い**
>
> つまり「完全に消去した」と言える構成は無い。**言えるのは取得不能にしたことだけである。**
> **層 1 の物理消滅はストレージ／バックアップの保持期間に従い、層 2 の物理消滅は保証しない。**

> **目標**: リソースの正本データを版として immutable 保持し（全フォーマット対象）、版の
> 一覧・取得・ダウンロードと、sysadmin による過去版パージを実装する。
...
- 軽量な移行（アップロードのみ copy + 行 insert でパイプライン非実行）は一般ユーザーには
...
- 公開リソース詳細（閲覧者向け）は本フェーズでは**版履歴を出さない**（差分サマリの提示は
  ...
  - **追記（ii-b 後に実装済み）**: 閲覧者向けの版履歴（一覧 + 版指定ダウンロード）を
    公開リソース詳細に追加した。折りたたみセクションで、開いたときに初めて取得し、
    既定は最新 10 件 + 「すべて表示」。削除済みの版は墓標（削除済みバッジ + 作成日時）
    として表示する。API は従来どおり可視性チェックのみで、匿名レスポンスに
    `publicCache()` を適用（ADR-026）。**差分の公開は引き続き Phase iii**（診断・
    パージは従来どおりダッシュボード側のみ）。
