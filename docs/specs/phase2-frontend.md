...
- [x] `pnpm build` が `apps/web` を含めて全パッケージで成功
- [x] `pnpm dev` で API + Web が同時起動
- [x] トップページ、データセット一覧/詳細、組織一覧/詳細、グループ一覧/詳細が表示
- [x] 全文検索が動作（キーワード入力 → 結果表示）
- [x] ログイン / ユーザー登録が動作
- [x] ダッシュボードにログイン後アクセス可能
- [x] データセット作成/編集/削除がフォームから操作可能
- [x] 組織/グループの作成が管理画面から操作可能
- [x] APIトークンの生成/削除がUIから操作可能
- [x] レスポンシブ対応（モバイル表示が崩れない）
- [x] `pnpm test` が全テスト合格（既存194 + 新規 = 361テスト）
