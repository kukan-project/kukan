...
| `brand`            | string                   | none (default brand `brands/default`)                   | Web image brand (`KUKAN_BRAND`). **When unset, the default brand (`brands/default`) is used.** If set, a matching `apps/web/brands/<brand>/` must exist (an unknown brand fails the image build) |
