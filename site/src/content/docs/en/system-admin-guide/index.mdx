...
The generation/embedding model can be switched from the admin dashboard above without redeploying. For **Bedrock**, though, the AI-suggestion model options are limited to `bedrock.completionModels` in `environments.ts` (the models granted `InvokeModel` in IAM); to add a candidate, list it there and redeploy (Ollama and OpenAI-compatible endpoints enumerate their available models automatically). To run Ollama on a GPU, see the [Getting Started guide](/en/getting-started/).
