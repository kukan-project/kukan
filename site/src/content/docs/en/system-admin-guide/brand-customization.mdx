...
| `--primary` / `--primary-foreground` | Main color and text on it |
| `--secondary` / `--muted` / `--accent` (each with `-foreground`) | Supporting colors |
| `--destructive` / `--destructive-foreground` | Errors and deletion (solid fill and text on it) |
| `--success` / `--warning` | Success / warning state colors (borders, tinted backgrounds, icons) |
| `--success-tint-foreground` / `--warning-tint-foreground` | Text color **on the tinted (/10–/15)** success / warning surfaces; not for solid fills |
| `--highlight` | Search keyword highlighting |
| `--border` / `--input` / `--ring` | Borders, input outlines, focus rings |
| `--radius` | Base border radius |
| `--kukan-*` | Layout values such as header height |
...
- **Do not modify upstream test files.** If a customization breaks an upstream test, report it to the upstream repository via an issue / PR instead of patching it in your fork
