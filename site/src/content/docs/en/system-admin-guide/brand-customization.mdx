...
Text fields (`siteName`, `siteDescription`, `copyright`, `logo.alt`, `headerNavExtra[].label`, `footerLinks[].label`) accept either a plain string or a locale map (e.g. `{ ja: '利用規約', en: 'Terms of Use' }`). A plain string applies to all locales; a map follows locale switching. Missing locales fall back to English (the default locale), then to any defined entry.
...
| ------------------------------------------------------- | ----------------------------------------------------- |
| Brand-specific values (site name, nav items, copyright) | `brand-config.ts` (locale maps for multilingual text) |
| Overriding the main UI's translated text                | `brand/messages/{locale}.json`                        |
...
When a custom component displays `brandConfig` text fields (`siteName` etc.), resolve them through `resolveBrandConfig()` first. Text fields are typed `LocalizedText` (a string or a locale map), so rendering them directly like `{brandConfig.siteName}` is a type error.

In async server components, use `getLocale()`:
...
export async function CustomHeader() {
  ...
  // brand.siteName is a string resolved for the current locale
  return <span>{brand.siteName}</span>
}
```

In client components, use `useLocale()` (do not import from `next-intl/server` — it is server-only and breaks the build). `useLocale()` also works in non-async server components.
...
:::caution[Custom components written before locale-map support]
Locale-map support widened the text field types from `string` to `LocalizedText`. Custom components that render these fields directly get type errors even if the config values are still plain strings. Switch to `resolveBrandConfig()` as shown above (`pnpm typecheck` lists every affected spot).
:::
