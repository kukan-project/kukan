...
`purgedAt` is filled in. Downloading it returns `404`. **The reason it was purged is not returned** —
it is free text about content that has been destroyed, and this endpoint is as readable as the
resource; who purged what, when and why is in the audit log.
