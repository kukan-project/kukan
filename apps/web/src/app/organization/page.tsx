  ...
  return (
    <div className="mx-auto max-w-[var(--kukan-container-max-width)] px-4 py-8">
      <div className="flex flex-col gap-6">
        ...
        <SearchForm defaultValue={q} action="/organization" placeholder="組織を検索..." />
              ...
              <Link key={org.id} href={`/organization/${org.name}`}>
                <Card className="h-full transition-colors hover:bg-accent/50">
                  <CardContent className="flex flex-col items-center gap-3 px-4 py-6 text-center">
                    <OrgImage imageUrl={org.imageUrl} name={org.title || org.name} />
          ...
          <div className="flex items-center justify-center gap-2">
            {offset > 0 && (
              <Button asChild variant="outline" size="sm">
                <Link href={`/organization?${buildQuery(q, offset - limit, limit)}`}>前へ</Link>
              </Button>
            )}
            ...
            {offset + limit < data.total && (
              <Button asChild variant="outline" size="sm">
                <Link href={`/organization?${buildQuery(q, offset + limit, limit)}`}>次へ</Link>
              </Button>
            )}
          </div>
        )}
...
function OrgImage({ imageUrl, name }: { imageUrl?: string | null; name: string }) {
  if (imageUrl) {
