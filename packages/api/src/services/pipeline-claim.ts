...
export async function releaseResourceClaims(
  ...
  const result = await db.execute(sql`
    ...
    SET claim_owner = NULL, claim_owner_at = NULL
...
export async function withResourceClaim<T>(
  ...
  const { claimed, held } = await claimResources(db, [resourceId], owner, CLAIM_STALE_AFTER_MS)
 ...
 * claims already taken are given back, because doing half of a purge would
...
export async function withResourceClaims<T>(
  ...
  const owner = randomUUID()
  const { claimed, held } = await claimResources(db, resourceIds, owner, CLAIM_STALE_AFTER_MS)
  const ids = claimed.map((c) => c.id)

  if (held.length > 0) {
    await releaseResourceClaims(db, ids, owner)
...
export async function withResourceClaimsOrConflict<T>(
  ...
  if (outcome.status === 'held') {
    throw new ConflictError(
      `Resource ${outcome.held[0].resourceId} is being processed; retry the purge`
    )
  }
